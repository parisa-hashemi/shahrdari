...
function ScientificNewProposalWizard() {
  ...
  // STEP 6: Confirmation Screen with Tracking Number
    ...
    <div style="max-width:680px;margin:40px auto;text-align:center">
      <div style="width:56px;height:56px;background:rgba(20,121,90,0.12);color:var(--ok);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px auto">
        ${ico.check}
      </div>
      ...
      <p class="muted" style="font-size:13.5px;line-height:1.6;margin-bottom:20px">
        پیشنهاد شما در دبیرخانه تحلیلی سامانه ثبت گردید و در کارتابل کارشناسان برنامه‌ریزی و تحلیل شهری قرار گرفت.
      </p>

      <div class="card" style="padding:18px;background:#F8FAFC;margin-bottom:24px;display:inline-block;min-width:320px">
        <span class="muted" style="font-size:12px;display:block;margin-bottom:4px">شماره پیگیری یکتا (Proposal ID):</span>
        <b class="code" style="font-size:18px;color:var(--brand)">${trackId}</b>
      </div>
    ...
    next.onclick = () => {
      ...
      if (s === 5) {
        ...
        logAct({
          ...
          etitle: d.title,
          to: 'submitted',
          text: `پیشنهاد علمی «${d.title}» توسط ${STATE.user?.name || 'نهاد علمی'} ثبت گردید`,
          k: 'ok'
        });
        saveDB();
        ...
        w.step = 6;
        render();
        toast('پیشنهاد علمی با موفقیت ثبت و وارد صف بررسی کارشناسی شد');
        return;
      }
...
function AnalystScientificProposalsPage() {
  ...
  const all = (DB.proposals || []).filter(p => p.src === 'academic' || p.id === 'PR-1405-000124' || p.id === 'PR-1405-000125');
  ...
  <section class="card">
    ...
    <div class="card-b" style="padding:0">
        ...
        <table class="tbl">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style="white-space:nowrap">
                <button class="btn btn-sm btn-pri" data-an-detail="${p.id}">${ico.spark}بررسی و ایجاد سناریو</button>
              </td>
            </tr>
            `).join('')}
          </tbody>
      ...
      <div style="padding:32px;text-align:center">
        <p class="muted">پیشنهادی با فیلترهای انتخابی یافت نشد.</p>
  ...
  els('[data-an-detail]').forEach(b => {
    b.onclick = () => {
      ...
      <div class="modal-b" style="max-height:75vh;overflow-y:auto">
        ...
        <div style="background:#F8FAFB;border:1px solid var(--line);padding:12px;border-radius:var(--r-m)">
          <b style="font-size:12px;display:block;margin-bottom:6px">اقدامات کارشناسی تحلیلگر:</b>
          <div style="display:flex;gap:10px;flex-wrap:wrap">
            <button class="btn btn-pri" id="an-do-create-scenario" style="flex:1;justify-content:center">
              ${ico.spark} ایجاد سناریوی تحلیلی رسمی از این پیشنهاد
            </button>
      ...
      // Action 1: Create Scenario from Proposal
      const createBtn = el('#an-do-create-scenario', modal);
      if (createBtn) {
        createBtn.onclick = () => {
          const scId = `SC-1405-PROP-${String(Math.floor(100 + Math.random() * 899))}`;
          const studyId = p.study || 'ST-1405-014';

          p.scenario_id = scId;
          p.status = 'ready';

          // Create Scenario entity in TUIP
          const newScenario = {
            id: scId,
            proposal_id: p.id, // Mandatory reference
            name: `سناریوی ارزیابی اثر: ${p.t || p.title} (پیشنهاد ${p.id})`,
            study: studyId,
            by: ME(),
            date: '۱۴۰۵/۰۶/۲۳',
            st: 'draft',
            baseline: `وضع موجود فصل صفر مطالعه ${studyId}`,
            modules: ['M01', 'M02', 'M03', 'M04', 'M05', 'M06', 'M07', 'M08', 'M10', 'M11', 'M12'],
            kpis: {
              ptrip: { b: 63000, s: 78500, unit: 'سفر در روز' },
              vtrip: { b: 37800, s: 26400, unit: 'سفر در روز' },
              co2: { b: 27720, s: 19800, unit: 'کیلوگرم CO₂e' }
            }
          };
          ...
          RUNS.unshift({
            ...
            p: 15,
            dur: 'در صف اجرا',
            note: `سناریوی برگرفته از پیشنهاد علمی ${p.id}`,
            by: ME()
          });

          logAct({
            action: 'CREATE_SCENARIO_FROM_PROPOSAL',
            et: 'scenario',
            eid: scId,
            etitle: newScenario.name,
            to: 'draft',
            text: `کارشناس تحلیل (${ME()}) از پیشنهاد علمی «${p.id}» سناریوی رسمی تحلیلی ایجاد کرد`,
            k: 'ok'
          });

          // Call backend API
          fetch(`/api/analyst/proposals/${p.id}/create-scenario`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'x-user-id': ME(),
          ...
          saveDB();
          closeAll();
          toast(`سناریوی تحلیلی ${scId} با ارجاع به پیشنهاد علمی ایجاد گردید`);
          render();
        };
      }
